...
class BackgroundDownloadService {
  ...
  Future<void> init() async {
    ...
    await service.configure(
      androidConfiguration: AndroidConfiguration(
        ...
        initialNotificationTitle: 'Taadia',
        initialNotificationContent: 'جاري تحميل صفحات القرآن...',
  ...
  static void _onStart(ServiceInstance service) async {
    ...
    try {
        ...
        await Future.wait(batch.map((page) async {
          for (var attempt = 0; attempt < 3; attempt++) {
            try {
              ...
              debugPrint('BackgroundDownload: failed page $page (attempt ${attempt + 1}): $e');
              if (attempt < 2) {
      ...
      debugPrint('BackgroundDownload: service crashed: $e');
      if (service is AndroidServiceInstance) {
        await service.setForegroundNotificationInfo(
          title: 'Taadia',
          content: 'Download error - tap to retry',
        );
      }
      service.invoke('update', {'error': true, 'message': e.toString()});
    }
  }
